...
ms:assetid: 'ms10-086'
ms:contentKeyID: 61234225
ms:mtpsurl: 'https://technet.microsoft.com/pt-BR/library/ms10-086(v=Security.10)'
---
...
Vulnerabilidade nos discos de cluster compartilhados no Windows pode permitir a falsificação (2294255)
------------------------------------------------------------------------------------------------------

Publicado: terça-feira, 12 de outubro de 2010 | Atualizado: quarta-feira, 10 de novembro de 2010
...
Esta atualização de segurança elimina uma vulnerabilidade em particular relatada no Windows Server 2008 R2 quando usado como um cluster de failover compartilhado. A vulnerabilidade pode permitir a falsificação de dados nos compartilhamentos administrativos de discos de cluster de failover. Por padrão, os servidores Windows Server 2008 R2 não são afetados por esta vulnerabilidade. Esta vulnerabilidade só se aplica a discos de cluster usados em um cluster de failover.

Esta é uma atualização de segurança classificada como Moderada para todas as edições do Windows Server 2008 R2 com suporte. Para obter mais informações, consulte a subseção **Software afetado e não afetado**, nesta seção.

A atualização de segurança elimina esta vulnerabilidade, mudando a maneira como a interface do usuário MSCS define as permissões em compartilhamentos administrativos ao adicionar ou criar novos discos de cluster compartilhados. Para obter mais informações sobre a vulnerabilidade, consulte a subseção Perguntas frequentes relacionada à entrada da vulnerabilidade específica presente na próxima seção, **Informações sobre a vulnerabilidade**.

**Recomendação**. A Microsoft recomenda que os clientes considerem aplicar a atualização de segurança usando o software de gerenciamento de atualização ou procurar atualizações usando o serviço [Microsoft Update](http://go.microsoft.com/fwlink/?linkid=40747).
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|---------------------------------|--------------------------------------------|
| [Windows Server 2008 R2 para sistemas baseados em x64](http://www.microsoft.com/downloads/details.aspx?familyid=1de12fdf-b439-4020-9313-a193d47dcfb2)\*   | Falsificação               | Moderada                        | Nenhuma                                    |
| [Windows Server 2008 R2 para sistemas baseados no Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=1c607c7d-6144-4a39-beea-a31b62085047) | Falsificação               | Moderada                        | Nenhuma                                    |
...
<span></span>
**Por que este boletim foi revisado em 10 de novembro de 2010?**  
A Microsoft revisou este boletim para indicar que a atualização corrige permissões em discos de cluster existentes compartilhados. Anteriormente, o boletim tinha declarado que a atualização não corrige permissões em discos de cluster existentes. Esta é apenas uma alteração informativa. Os clientes que já atualizaram seus sistemas com êxito não precisam fazer mais nada.

**Essa atualização de segurança corrige permissões em discos de cluster existentes compartilhados?**  
Sim, ao instalar a atualização de segurança em todos os nós de clusters e reiniciar os computadores fará com que as permissões em discos de cluster existentes compartilhados sejam corrigidas.
...
<th style="border:1px solid black;" >Vulnerabilidade de permissões em novos discos de cluster - CVE-2010-3223</th>
...
Vulnerabilidade de permissões em novos discos de cluster - CVE-2010-3223  
------------------------------------------------------------------------
  
<span></span>
Existe uma vulnerabilidade de falsificação na maneira como a interface do usuário do Gerenciador de cluster de failover manipula permissões nos discos de cluster compartilhados. Esta vulnerabilidade existe porque o Gerenciador de cluster de failover usa as permissões padrão não seguras ao adicionar discos a um cluster. Quando um administrador adiciona um disco a um cluster compartilhado, o Gerenciador de cluster de failover define as permissões no disco de cluster compartilhado de uma forma que fornece acesso potencial de leitura/gravação/exclusão a usuários não autorizados (todos) aos compartilhamentos administrativos no disco de cluster de failover.
  
Para exibir esta vulnerabilidade como uma entrada padrão na lista Common Vulnerabilities and Exposures, consulte [CVE-2010-3223](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3223).
  
#### Fatores atenuantes para a vulnerabilidade de permissões em novos discos de cluster - CVE-2010-3223
  
A Microsoft não identificou nenhum fator atenuante para essa vulnerabilidade.
  
#### Soluções alternativas para a vulnerabilidade de permissões em novos discos de cluster - CVE-2010-3223
  
A solução alternativa refere-se a uma configuração ou alteração de configuração que não corrige a vulnerabilidade subjacente mas que ajudaria a bloquear vetores de ataque conhecidos antes de aplicar a atualização. A Microsoft testou as soluções alternativas e estados a seguir como parte do cenário que implica se uma solução alternativa reduziria funcionalidade:
  
-   Depois de criar um compartilhamento administrativo de disco de cluster, defina manualmente sua Lista de controle de acesso (ACL) para refletir as permissões de acesso adequadas ao ambiente. Por exemplo, mude as permissões nos compartilhamentos administrativos de Todos - Controle Total para Administradores - Controle Total.  
-   Os administradores do servidor também podem optar por clusterizar novamente os discos afetados depois que a atualização é aplicada. Isso define corretamente as permissões nos novos discos de cluster compartilhados.
  
#### Perguntas frequentes sobre a vulnerabilidade de permissões em novos discos de cluster - CVE-2010-3223
  
**Qual é o escopo da vulnerabilidade?**    
Essa é uma vulnerabilidade de falsificação. Um invasor que explorar com êxito esta vulnerabilidade poderá adicionar, alterar, excluir ou ler dados arbitrariamente em um sistema de arquivos afetado.
  
**O que provoca a vulnerabilidade?**    
Esta vulnerabilidade é causada pela maneira como o Windows define permissões por padrão ao adicionar um disco a um cluster de disco.
  
**Para que são usados os compartilhamentos administrativos?**    
Os compartilhamentos administrativos são os compartilhamento padrão de rede criados pelos sistemas operacionais Windows. Por padrão, esses compartilhamentos permitem que qualquer pessoa que seja membro do grupo Administradores gerencie o diretório raiz dos discos anexados ao sistema.
  
**Qual componente é afetado por esta vulnerabilidade?**    
O componente afetado por esta vulnerabilidade é o MSCS (Microsoft Cluster Service).
  
**Para que um invasor pode usar a vulnerabilidade?**    
Um invasor que explorar com êxito esta vulnerabilidade pode adicionar, alterar, excluir ou ler dados nos compartilhamentos administrativos de uma configuração de disco de cluster afetada.
  
**De que forma o invasor pode explorar a vulnerabilidade?**    
Um invasor pode acessar remotamente o sistema de arquivos de um compartilhamento administrativo de disco de cluster.
  
**Quais são os principais sistemas que correm riscos com a vulnerabilidade?**    
Os sistemas nos quais o cluster de failover foi ativado são os que mais correm risco com esta vulnerabilidade. Os servidores que não fazem parte de um cluster de failover não são afetados por esta vulnerabilidade e a eles não será oferecida atualização.
  
**O que a atualização faz?**    
Esta atualização elimina esta vulnerabilidade, mudando a maneira como a interface do usuário MSCS define as permissões nos compartilhamentos administrativos ao adicionar novos discos de cluster compartilhados.
  
Para obter mais informações, consulte a entrada nas Perguntas frequentes, "Essa atualização de segurança corrige permissões em discos de cluster existentes compartilhados?", na seção anterior, **Perguntas frequentes relacionadas a esta atualização de segurança**.
...
<td style="border:1px solid black;">Para todas as edições baseadas em x64 do Windows Server 2008 R2 com suporte com uma configuração MSCS:<br />
windows6.1-kb2294255-x64.msu /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Para todas as edições baseadas no Itanium do Windows Server 2008 R2 com suporte com uma configuração MSCS:<br />
...
<td style="border:1px solid black;">Consulte o <a href="http://support.microsoft.com/kb/2294255">Artigo 2294255 (em inglês) da Microsoft Knowledge Base</a></td>
</tr>
<tr class="odd">
...
-   V1.1 (10 de novembro de 2010): Atualização de Perguntas frequentes adicionada para explicar que esta atualização corrige as permissões em discos de cluster existentes compartilhados. Esta é apenas uma alteração informativa. Os clientes que já atualizaram seus sistemas com êxito não precisam fazer mais nada.
