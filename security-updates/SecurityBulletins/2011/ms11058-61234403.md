...
ms:assetid: 'ms11-058'
ms:contentKeyID: 61234403
ms:mtpsurl: 'https://technet.microsoft.com/pt-BR/library/ms11-058(v=Security.10)'
---
...
Vulnerabilidades no servidor DNS podem permitir a execução remota de código (2562485)
-------------------------------------------------------------------------------------

Publicado: terça-feira, 9 de agosto de 2011 | Atualizado: terça-feira, 25 de outubro de 2011
...
Esta atualização de segurança elimina duas vulnerabilidades relatadas em particular no servidor Windows DNS. A mais severa destas vulnerabilidades pode permitir execução remota de código se um invasor registrar um domínio, criar um registro de recurso do DNS de NAPTR, e então enviar uma consulta de NAPTR especialmente criada para servidor de DNS de destino. Os servidores que não têm a função de DNS habilitada não estão em risco.

Esta atualização de segurança é classificada como Crítica para as edições com suporte de 32 bits e baseadas em x64 do Windows Server 2008 e edições baseadas em x64 do Windows Server 2008 R2; e classificada como Importante para todas as edições com suporte do Windows Server 2003. Para obter mais informações, consulte a subseção **Softwares afetados e não afetados**, nesta seção.

A atualização de segurança aborda as vulnerabilidades modificando a maneira como servidor DNS manipula consultas de NAPTR em memória e inicializa objetos em memória antes do uso. Para obter mais informação sobre as vulnerabilidades, consulte a subseção Perguntas freqüentes relacionada à entrada da vulnerabilidade específica presente na próxima seção, **Informações sobre a vulnerabilidade**.
...
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|---------------------------------|---------------------------------------------------------------------------|
| [Windows Server 2003 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=c2d4aa38-9241-465d-8d65-aba73e936d0f)                                                                                            | Negação de Serviço         | Importante                      | [MS09-008](http://technet.microsoft.com/pt-br/security/bulletin/ms09-008) |
| [Windows Server 2003 x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=2db4098a-f4f9-4211-b55d-5d0df1409c43)                                                                                | Negação de Serviço         | Importante                      | [MS09-008](http://technet.microsoft.com/pt-br/security/bulletin/ms09-008) |
| [Windows Server 2003 com SP2 para sistemas baseados no Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=1934acfa-5637-4ae9-9e9a-fc7e58930b7a)                                                                 | Negação de Serviço         | Importante                      | [MS09-008](http://technet.microsoft.com/pt-br/security/bulletin/ms09-008) |
| [Windows Server 2008 para sistemas de 32 bits Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=ac2d5122-6f9b-46f5-9840-77aa7ff84308)&#42;                                                                 | Execução Remota de Código  | Crítica                         | Nenhuma                                                                   |
| [Windows Server 2008 para sistemas baseados em x64 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=5f605f6f-3854-403d-878b-637626aef78f)&#42;                                                            | Execução Remota de Código  | Crítica                         | Nenhuma                                                                   |
| [Windows Server 2008 R2 para sistemas baseados em x64 e Windows Server 2008 R2 para sistemas baseados em x-64 Service Pack 1](http://www.microsoft.com/downloads/details.aspx?familyid=6a86e2cf-4e7d-4012-88c3-6957a3842dd3)&#42; | Execução Remota de Código  | Crítica                         | Nenhuma                                                                   |
...
**Por que este boletim foi revisado em 25de outubro de 2011?**  
A Microsoft revisou este boletim para anunciar uma alteração à lógica de detecção e corrigir as informações de substituição de boletim para todas as edições com suporte do Windows Server 2003. Não houve nenhuma alteração para os arquivos de atualização de segurança. Os clientes que já atualizaram seus sistemas com êxito não precisam fazer mais nada.
...
<th style="border:1px solid black;" >Perguntas frequentes sobre a Vulnerabilidade de consulta do NAPTR do DNS - CVE-2011-1966</th>
<th style="border:1px solid black;" >Vulnerabilidade de corrupção de memória não inicializada do DNS - CVE-2011-1970</th>
...
Perguntas frequentes sobre a Vulnerabilidade de consulta do NAPTR do DNS - CVE-2011-1966  
----------------------------------------------------------------------------------------
  
Uma vulnerabilidade de execução remota de código existe na maneira como o servidor de DNS do Windows manipula impropriamente uma sequência de caracteres de consulta de NAPTR especialmente criada em memória. O invasor que explorar esta vulnerabilidade com êxito poderá executar código arbitrário no contexto do sistema. O invasor poderá instalar programas; exibir, alterar ou excluir dados; ou criar novas contas com direitos totais de usuário.
  
Para exibir esta vulnerabilidade como uma entrada padrão na lista Common Vulnerabilities and Exposures, consulte CVE-2011-1966.
  
#### Fatores atenuantes da Vulnerabilidade de consulta do NAPTR do DNS - CVE-2011-1966
  
A Microsoft não identificou nenhum fator atenuante para essa vulnerabilidade.
  
#### Soluções alternativas para Vulnerabilidade de consulta do NAPTR do DNS - CVE-2011-1966
  
A solução alternativa refere-se a uma configuração ou alteração de configuração que não corrige a vulnerabilidade subjacente mas que ajudaria a bloquear vetores de ataque conhecidos antes de aplicar a atualização. A Microsoft testou as soluções alternativas e estados a seguir como parte do cenário que implica se uma solução alternativa reduziria funcionalidade:
  
-   **Desabilite o serviço de DNS se você não o usa**
  
    **Métodointerativo:**
  
    1.  Clique em **Iniciar**, **Executar**, digite **regedit** e clique em **OK**.  
    2.  Expanda **HKEY\_LOCAL\_MACHINE**, Expanda **System**, expanda **CurrentControlSet** e depois **Services**.  
    3.  Clique em **DNS**.  
    4.  Clique no menu **Arquivo** e, em seguida, selecione **Exportar**.  
    5.  Na caixa de diálogo Exportar Arquivo do Registro, digite **DNS\_configuration\_backup.reg** e clique em **Salvar**.  
    6.  Clique duas vezes no valor chamado **Iniciar** e então altere o campo de dados **de Valor** para **4**.  
    7.  Clique em **OK**.  
    ...
    **Usando um script de implantação gerenciado:**
    ...
    1.  Copie as chaves do Registro de um script de implantação gerenciado usando o seguinte comando:  
        `regedit/e DNS_configuration_backup.reg HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\DNS`  
    2.  Salve o seguinte texto em um arquivo com extensão .REG (por exemplo: "Disable\_service\_DNSame.reg"):  
        `Windows Registry Editor Versão 5.00`  
        `[HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\DNS]`  
        `"Start"=dword:00000004`  
    3.  Execute o script de Registro criado na etapa 2 acima na máquina de destino com o seguinte comando:  
    ...
    **Métodointerativo:**
    ...
    3.  Na caixa de diálogo **Importar arquivo do Registro**, selecione **DNS\_configuration\_backup.reg** e clique em Abrir.
  
    **Usando um script de implantação gerenciado:**
  
    Restaure o registro a seu estado original executando o seguinte comando num prompt de comando como um administrador:  
...
A vulnerabilidade é causada quando um servidor não-autorizado de DNS impropriamente manipula um registro de NAPTR especialmente criado em memória ao recursivamente solicitar o registro do servidor autoritário de DNS.
  
**Qual componente é afetado por esta vulnerabilidade?**   
Dns.exe é o componente afetado por esta vulnerabilidade.
  
**Para que um invasor pode usar a vulnerabilidade?**   
O invasor que explorar esta vulnerabilidade com êxito poderá executar código arbitrário no contexto do sistema. O invasor poderá instalar programas; exibir, alterar ou excluir dados; ou criar novas contas com direitos totais de usuário.
  
**De que forma o invasor pode explorar a vulnerabilidade?**   
Um invasor não autenticado remoto pode explorar esta vulnerabilidade registrando um domínio, criando um registro de recurso do DNS de NAPTR, e então enviando uma consulta de NAPTR especialmente criada para servidor de DNS de destino.
  
**Quais são os sistemas que mais correm riscos com a vulnerabilidade?**   
Os servidores que executam a função do DNS são os que mais correm risco. Os servidores que não têm a esta função habilitada não estão em risco.
  
**O que é um registro de recurso NAPTR de DNS?**   
Um registro de recurso de DNS (RR) do Naming Authority Pointer (NAPTR) permite que o DNS seja usado para pesquisar serviços de uma variedade de nomes de recursos, inclusive URI que não estão em sintaxe de nome de domínio. Para obter mais informações sobre NAPTR, consulte RFC-2915.
  
**O que é um registro de recurso DNS?**   
Um registro de recurso de DNS (RR) identifica um recurso específico dentro de um domínio. Para obter mais informações, consulte o artigo do TechNet How DNS Works (em inglês).
...
A atualização de segurança elimina a vulnerabilidade modificando a maneira como o servidor DNS manipula consultas de NAPTR na memória.
...
Vulnerabilidade de corrupção de memória não inicializada do DNS - CVE-2011-1970  
-------------------------------------------------------------------------------
  
Uma vulnerabilidade de negação de serviço existe na maneira como o servidor de DNS manipula impropriamente um objeto em memória que não foi inicializado. O invasor que explorar com êxito esta vulnerabilidade poderá fazer com que o serviço do servidor DNS pare de responder no sistema de destino.
  
Para exibir esta vulnerabilidade como uma entrada padrão na lista Common Vulnerabilities and Exposures, consulte CVE-2011-1970.
  
#### Fatores atenuantes da Vulnerabilidade de corrupção de memória não inicializada de DNS - CVE-2011-1970
  
A Microsoft não identificou nenhum fator atenuante para essa vulnerabilidade.
  
#### Soluções alternativas para a vulnerabilidade de corrupção de memória não inicializada de DNS - CVE-2011-1970
  
A Microsoft não identificou nenhuma solução alternativa para esta vulnerabilidade.
  
#### Perguntas frequentes sobre a vulnerabilidade de corrupção de memória não inicializada de DNS - CVE-2011-1970
  
**Qual é o escopo da vulnerabilidade?**   
Essa é uma vulnerabilidade de negação de serviço (site em inglês).
  
**O que provoca a vulnerabilidade?**   
A vulnerabilidade é causada quando o servidor de DNS impropriamente manipula um objeto não inicializado na memória ao pesquisar um registro do recurso de um domínio inexistente.
  
**Qual componente é afetado por esta vulnerabilidade?**   
Dns.exe é o componente afetado por esta vulnerabilidade.
  
**Para que um invasor pode usar a vulnerabilidade?**   
O invasor que explorar com êxito esta vulnerabilidade poderá fazer com que o serviço do servidor DNS pare de responder no sistema de destino.
  
**De que forma o invasor pode explorar a vulnerabilidade?**   
Um invasor não autenticado remoto pode explorar esta vulnerabilidade enviando uma consulta de DNS especialmente criada ao servidor DNS de destino para o registro de recurso de um domínio inexistente.
...
**O que a atualização faz?**   
A atualização de segurança elimina a vulnerabilidade modificando a maneira como o servidor DNS inicializa objetos na memória antes de usá-los.
...
<td style="border:1px solid black;">Use a ferramenta <strong>Adicionar ou Remover Programas</strong> no Painel de Controle ou o utilitário Spuninst.exe, localizado na pasta %Windir%\$NTUninstallKB2562485$\Spuninst</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>Informaçõessobre</strong> <strong>o</strong> <strong>arquivo</strong></td>
<td style="border:1px solid black;">Consulte o Artigo 2562485 (em inglês) da Microsoft Knowledge Base</td>
</tr>
<tr class="even">
...
-   Grischa Zengel (Zengel Medizintechnik GmbH) por relatar a Vulnerabilidade de consulta de NAPTR de DNS (CVE-2011-1966)
...
-   V1.1 (21 de setembro de 2011): Foi corrigida a tabela de Softwares afetados para remover o MS11-046 como um boletim substituído por esta atualização em todas as edições afetadas do Windows Server 2003. Esta é apenas uma alteração de detecção. Não houve nenhuma alteração para os arquivos de atualização de segurança. Os clientes que já atualizaram seus sistemas com êxito não precisam fazer mais nada.  
-   V1.2 (25 de outubro de 2011): Foi anunciada uma alteração na lógica de detecção e foram corrigidas as informações de substituição de boletim para algumas configurações afetadas. Não houve nenhuma alteração para os arquivos de atualização de segurança. Consulte as Perguntas frequentes de atualização para obter detalhes.
