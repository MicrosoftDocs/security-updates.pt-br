...
ms:assetid: 'ms11-062'
ms:contentKeyID: 61234407
ms:mtpsurl: 'https://technet.microsoft.com/pt-BR/library/ms11-062(v=Security.10)'
---
...
Vulnerabilidade no driver NDISTAPI de serviço de acesso remoto pode permitir elevação de privilégio (2566454)
-------------------------------------------------------------------------------------------------------------
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|---------------------------------|--------------------------------------------|
| [Windows XP Service Pack 3](http://www.microsoft.com/downloads/details.aspx?familyid=6bc1f0ac-223d-4b0b-86cd-2ba3863955c4)                                     | Elevação de privilégio     | Importante                      | Nenhuma                                    |
| [Windows XP Professional x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=d5ee6787-408d-4870-af70-9338158b161b)            | Elevação de privilégio     | Importante                      | Nenhuma                                    |
| [Windows Server 2003 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=8de0f2db-aa09-48cd-a13b-e26a973e9cdc)                            | Elevação de privilégio     | Importante                      | Nenhuma                                    |
| [Windows Server 2003 x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=8f208407-4bb3-41fe-b0d6-fc8a5e30e667)                | Elevação de privilégio     | Importante                      | Nenhuma                                    |
| [Windows Server 2003 com SP2 para sistemas baseados no Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=ef140089-d022-4ee8-9cc4-7fb25295a39d) | Elevação de privilégio     | Importante                      | Nenhuma                                    |
...
<th style="border:1px solid black;" >Vulnerabilidade de elevação de privilégio de NDISTAPI - CVE-2011-1974</th>
...
Vulnerabilidade de elevação de privilégio de NDISTAPI - CVE-2011-1974  
---------------------------------------------------------------------
  
<span></span>
Existe uma elevação de vulnerabilidade de privilégio no driver NDISTAPI de serviço de acesso remoto. A vulnerabilidade é causada quando o driver de NDISTAPI impropriamente valida a entrada fornecida pelo usuário ao passar dados do modo de usuário ao kernel do Windows. Um invasor local que explore com êxito esta vulnerabilidade pode causar a execução de código arbitrário e assumir o controle total do sistema afetado. O invasor poderá instalar programas; exibir, alterar ou excluir dados; ou criar novas contas com direitos totais de usuário.
  
Para exibir esta vulnerabilidade como uma entrada padrão na lista Common Vulnerabilities and Exposures, consulte [CVE-2011-1974](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1974).
  
#### Fatores atenuantes da vulnerabilidade de elevação de privilégio de NDISTAPI - CVE-2011-1974
...
#### Soluções alternativas para vulnerabilidade de elevação de privilégio de NDISTAPI - CVE-2011-1974
  
A Microsoft não identificou nenhuma solução alternativa para esta vulnerabilidade.
  
#### Perguntas frequentes sobre vulnerabilidade de elevação de privilégio de NDISTAPI - CVE-2011-1974
  
**Qual é o escopo da vulnerabilidade?**    
Esta é uma vulnerabilidade de elevação de privilégio.
  
**O que provoca a vulnerabilidade?**    
A vulnerabilidade é causada quando o driver de NDISTAPI impropriamente valida a entrada fornecida pelo usuário ao passar dados do modo de usuário ao kernel do Windows.
  
**Qual componente é afetado pela vulnerabilidade?**    
NDISTAPI.sys é o componente afetado pela vulnerabilidade.
  
**O que é o Serviço de acesso remoto (RAS)?**    
O Serviço de acesso remoto (RAS) permite que os usuários se conectem a um computador remoto por meio de linhas telefônicas, da Internet ou de outra conexão de rede para poder trabalhar como se o sistema estivesse fisicamente conectado à rede remota. O RAS permite que os usuários executem determinadas atividades, como enviar e-mails, enviar documentos via fax, recuperar arquivos e imprimir documentos em uma impressora no escritório. Para obter mais informações, consulte o artigo do MSDN, [RAS](http://msdn.microsoft.com/en-us/library/ff563285(v=vs.85).aspx) Architecture Overview (NDIS 5.1).
  
RAS é um serviço nativo no Windows XP e Windows Server 2003. Este serviço foi substituído pelo serviço Roteamento e Acesso Remoto (RRAS) em sistemas operacionais posteriores. O RRAS possibilita a um computador funcionar como um roteador de rede. O RRAS também fornece a próxima geração de funcionalidades de servidor para RAS para Windows. A funcionalidade de servidor RRAS segue e se baseia na funcionalidade RAS incluída em sistemas operacionais anteriores.
  
**O que é o driver de NDISTAPI?**    
O driver de NDISTAPI é parte da arquitetura de RAS e conecta o NDISWAN a serviços de TAPI. Aplicativos de telefonia implementam os serviços TAPI e podem fazer solicitações de TAPI para configurar, monitorar e derrubar chamadas e linhas. O driver de NDISTAPI transmite estas solicitações de TAPI ao NDISWAN. Para obter mais informações, consulte o artigo do MSDN, [NDISTAPI](http://msdn.microsoft.com/en-us/library/ff554813(v=vs.85).aspx) Overview (NDIS 5.1).
  
**O que é NDISWAN?**    
Network Driver Interface Standard Wide Area Network (NDISWAN) é um driver intermediário de NDIS fornecido pelos sistema que oferece funcionalidades, como compreensão de dados, criptografia, loopback e enquadramento de PPP simples usado por drivers de miniporta de WAN. Drivers de miniporta WAN são solicitados a implementar apenas os recursos específicos ao médio (por exemplo, sinalização Q931 é solicitada para ISDN). Para obter mais informações, consulte o artigo MSDN,[NDISWAN Overview](http://msdn.microsoft.com/en-us/library/ff564653(v=vs.85).aspx).
  
**O que é o serviço TAPI?**    
O serviço Interface de Programação de Aplicativo de Telefonia (TAPI) é uma interface de programação que oferece serviços de telefonia a aplicativos habilitados para comunicação telefônica.
  
**O que é NDIS?**    
Network Driver Interface Standard (NDIS) é parte do Kit de driver do Windows, que é um sistema de desenvolvimento de driver totalmente integrado que contem o Kit de dispositivo de driver do Windows e testa a estabilidade e confiabilidade dos Drivers do Windows. A NDIS Library abstrai o hardware de rede dos drivers de rede. NDIS também especifica uma interface padrão entre drivers de rede de camadas, abstraindo drivers de baixo nível que gerenciam hardware de drivers de nível superior, como transportes de rede. NDIS também mantém informações de estado e parâmetros para drivers de rede, inclusive ponteiros a funções, alças e blocos de parâmetro para conexão, além de outros valores de sistema.
  
**Para que um invasor pode usar a vulnerabilidade?**    
O invasor que explorar esta vulnerabilidade com êxito poderá executar código arbitrário no contexto do sistema local. O invasor poderá instalar programas; exibir, alterar ou excluir dados; ou criar novas contas com direitos totais de usuário.
...
A atualização de elimina a vulnerabilidade corrigindo a maneira como o driver NDISTAPI valida a entrada de modo do usuário antes de enviá-la ao kernel do Windows.
...
<td style="border:1px solid black;">Use a ferramenta <strong>Adicionar ou Remover Programas</strong> no Painel de Controle ou o utilitário Spuninst.exe, localizado na pasta %Windir%\$NTUninstallKB2566454$\Spuninst</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>Informações sobre o arquivo</strong></td>
<td style="border:1px solid black;">Consulte o <a href="http://support.microsoft.com/kb/2566454">Artigo 2566454 (em inglês) da Microsoft Knowledge Base</a></td>
</tr>
<tr class="even">
...
-   Lufeng Li da para relatar a Vulnerabilidade de elevação de privilégio de NDISTAPI (CVE-2011-1974)
